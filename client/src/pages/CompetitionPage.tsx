...
export const CompetitionPage: React.FC = () => {
  ...
  const fetchAIHint = async () => {
    ...
    try {
      const data = await apiRequest(`/questions/${activeQuestion._id}/hint`, {
        method: 'POST',
      });
      ...
      // Refresh score since points were deducted
      fetchCompetitionState();
    } catch (err: any) {
      setHintError(err.message || 'Hint service temporarily unavailable.');
    } finally {
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-800">
            ...
            <div className="flex items-center space-x-3">
              ...
              <button
                id="hint-request-btn"
                onClick={fetchAIHint}
                title={`Request a hint — costs ${HINT_POINT_COST} pts (max ${HINT_MAX} per question, unlocks after 2 wrong attempts)`}
              ...
              >
                ...
                <span>Hint <span className="font-mono text-purple-400 text-xs">−{HINT_POINT_COST}pts</span></span>
              </button>
            </div>
          </div>
          ...
          <div className="glass-panel max-w-lg w-full p-6 rounded-3xl border border-purple-500/40 shadow-2xl relative space-y-4">
            <div className="flex items-center space-x-3">
              <div className="w-10 h-10 rounded-xl bg-purple-600/20 border border-purple-500/30 flex items-center justify-center">
                <Sparkles className={`w-5 h-5 text-amber-400 ${hintLoading ? 'animate-spin' : ''}`} />
              </div>
              <div>
                <h3 className="text-lg font-bold text-white">AI Assistant Hint</h3>
                <p className="text-xs text-slate-400">Powered by Gemini AI · costs {HINT_POINT_COST} pts per use</p>
              </div>
            </div>
            ...
            <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 min-h-[80px] flex items-center">
              {hintLoading ? (
                <div className="flex items-center space-x-3 text-slate-400 text-sm">
                  ...
                  <span>Consulting AI model for a non-spoiler hint...</span>
                </div>
              ) : hintError ? (
                <p className="text-rose-400 text-sm leading-relaxed">{hintError}</p>
              ) : (
                <p className="text-slate-200 text-sm leading-relaxed font-mono">{hintText}</p>
              )}
            </div>

            {/* Hint metadata (shown after successful hint) */}
            {hintMeta && (
              <div className="grid grid-cols-3 gap-3 text-center">
                <div className="p-3 rounded-xl bg-rose-500/10 border border-rose-500/20">
                  <div className="text-xs font-mono text-rose-400 uppercase">Deducted</div>
                  <div className="text-lg font-bold text-rose-300">−{hintMeta.pointsDeducted} pts</div>
                </div>
                <div className="p-3 rounded-xl bg-purple-500/10 border border-purple-500/20">
                  <div className="text-xs font-mono text-purple-400 uppercase">Used</div>
                  <div className="text-lg font-bold text-purple-300">{hintMeta.hintsUsed} / {HINT_MAX}</div>
                </div>
                <div className="p-3 rounded-xl bg-slate-800 border border-slate-700">
                  <div className="text-xs font-mono text-slate-400 uppercase">Remaining</div>
                  <div className={`text-lg font-bold ${hintMeta.hintsRemaining > 0 ? 'text-slate-200' : 'text-red-400'}`}>
                    {hintMeta.hintsRemaining}
                  </div>
                </div>
              </div>
            )}
            ...
            <button
              id="hint-close-btn"
              onClick={() => { setHintModalOpen(false); setHintMeta(null); setHintError(null); }}
